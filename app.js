...
const STATUS = {
  // Monochroom: het onderscheid zit in de helderheid, van fel (vraagt
  // actie) naar gedempt (afgehandeld of vervallen).
  draft: { label: "Concept", color: "#6E6E6E" },
  active: { label: "Actief", color: "#FFFFFF" },
  finished: { label: "Afgerond", color: "#8A8A8A" },
  scheduled: { label: "Gepland", color: "#D4D4D4" },
  in_progress: { label: "Bezig", color: "#FFFFFF" },
  pending_confirmation: { label: "Ter bevestiging", color: "#FFFFFF" },
  confirmed: { label: "Bevestigd", color: "#9A9A9A" },
  cancelled: { label: "Geannuleerd", color: "#5C5C5C" },
};
...
function statCard({ label, value, ico = "trend", color = "#FFFFFF" }) {
  return `
    <div class="stat">
      <div class="ico" style="background:#1C1C1C;color:${color}">${icon[ico]}</div>
...
function errorView(error) {
  return `
    <div class="state">
      <div class="state-ico" style="color:#FFFFFF">${icon.warn}</div>
...
async function viewHome() {
  ...
  setView(`
    ...
    <div class="grid" style="grid-template-columns:repeat(2,1fr)">
      ...
      ${statCard({ label: "Gewonnen", value: winPct + "%", ico: "trophy" })}
    </div>
...
async function viewStats() {
  ...
  setView(`
    ...
    <div class="grid">
      ...
      ${statCard({ label: "Gewonnen", value: s.matches_won, ico: "trophy" })}
      ${statCard({ label: "Verloren", value: s.matches_lost, ico: "flag" })}
      ...
      ${statCard({ label: "180's", value: s.count_180, ico: "star" })}
    </div>
  ...
  setView(`
    ...
    <div class="card center">
      ...
      <div class="mt8">${p.role === "organizer"
        ? `<span class="badge" style="color:var(--accent);border-color:#F47B2066;background:#F47B201F">Organisator</span>`
        : `<span class="badge" style="color:#B5B5B5;border-color:#2A2A2A;background:#161616">Speler</span>`}</div>
    ...
    <div class="grid">
      ...
      ${statCard({ label: "Gewonnen", value: s?.matches_won ?? 0, ico: "trophy" })}
      ${statCard({ label: "Gemiddelde", value: Number(s?.average_score ?? 0).toFixed(1), ico: "trend" })}
      ${statCard({ label: "180's", value: s?.count_180 ?? 0, ico: "star" })}
    </div>
...
async function viewOrganizer() {
  ...
  setView(`
    ...
    <div class="grid">
      ...
      ${statCard({ label: "Open wedstrijden", value: c.open, ico: "match" })}
    </div>
