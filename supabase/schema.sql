...
--  13. Per-wedstrijd beschikbaarheid (available_at)
...
-- worden wedstrijden aangemaakt (zie sectie 11, activate_league).
create table if not exists public.leagues (
  ...
  -- Aantal legs dat een wedstrijd in deze league telt. Een wedstrijd is
  -- beslist zodra een speler de meerderheid wint (zie
  -- report_league_match_result, sectie 18) - niet alle legs hoeven gespeeld
  -- te worden. Een gelijkspel (bv. 5-5 bij 10) blijft mogelijk.
...
-- krijgt zijn eigen available_at (zie sectie 13) - dit hangt nooit af van
-- een andere wedstrijd. Intern (geen grant naar
      ...
      for i in 1 .. n / 2 loop
        ...
        if a is not null and b is not null then
          insert into public.league_matches
            (league_id, division_id, player_a_id, player_b_id, round_number, scheduled_at, available_at, status)
          values
            (p_league_id, div.division_id, a, b, rnd + 1, v_available, v_available, 'scheduled')
...
--     Deze sectie had oorspronkelijk ook een deadline/herinnering/verlenging-
--     mechanisme; dat is in sectie 22 afgeschaft.)
-- ============================================================================
...
-- organisator), niet alleen via de app - vergelijkbaar met
-- protect_league_schedule_fields voor leagues.
...
-- Wedstrijddeadlines (match_deadline_days/deadline_at/deadline_expired_at/
-- reminder_sent_at, extend_match_deadline(), process_match_deadlines()) zijn
-- verwijderd: spelers hebben geen vaste speeltermijn meer per wedstrijd,
-- alleen nog available_at (wanneer een wedstrijd beschikbaar komt). Zie de
-- kolomverwijdering en cron.unschedule verderop in dit bestand.
...
-- ============================================================================
-- 18. Uitgebreide wedstrijdstatistieken (Scoring, Eerste 9 Gem., Checkouts,
--     Worpen, Beste leg, 60+/80+/100+/140+) naast de bestaande Gemiddelde/
--     180's/Hoogste finish - mirrort het stat-overzicht dat spelers al kennen
--     uit Scolia/DartCounter. Allemaal optioneel.
-- ============================================================================

alter table public.league_matches
  add column if not exists player_a_scoring_average numeric(5,2),
  add column if not exists player_b_scoring_average numeric(5,2),
  add column if not exists player_a_first9_average numeric(5,2),
  add column if not exists player_b_first9_average numeric(5,2),
  add column if not exists player_a_checkouts_hit int check (player_a_checkouts_hit is null or player_a_checkouts_hit >= 0),
  add column if not exists player_b_checkouts_hit int check (player_b_checkouts_hit is null or player_b_checkouts_hit >= 0),
  add column if not exists player_a_checkout_attempts int check (player_a_checkout_attempts is null or player_a_checkout_attempts >= 0),
  add column if not exists player_b_checkout_attempts int check (player_b_checkout_attempts is null or player_b_checkout_attempts >= 0),
  add column if not exists player_a_darts_thrown int check (player_a_darts_thrown is null or player_a_darts_thrown >= 0),
  add column if not exists player_b_darts_thrown int check (player_b_darts_thrown is null or player_b_darts_thrown >= 0),
  add column if not exists player_a_best_leg_darts int check (player_a_best_leg_darts is null or player_a_best_leg_darts >= 0),
  add column if not exists player_b_best_leg_darts int check (player_b_best_leg_darts is null or player_b_best_leg_darts >= 0),
  add column if not exists player_a_score_60_plus int check (player_a_score_60_plus is null or player_a_score_60_plus >= 0),
  add column if not exists player_b_score_60_plus int check (player_b_score_60_plus is null or player_b_score_60_plus >= 0),
  add column if not exists player_a_score_80_plus int check (player_a_score_80_plus is null or player_a_score_80_plus >= 0),
  add column if not exists player_b_score_80_plus int check (player_b_score_80_plus is null or player_b_score_80_plus >= 0),
  add column if not exists player_a_score_100_plus int check (player_a_score_100_plus is null or player_a_score_100_plus >= 0),
  add column if not exists player_b_score_100_plus int check (player_b_score_100_plus is null or player_b_score_100_plus >= 0),
  add column if not exists player_a_score_140_plus int check (player_a_score_140_plus is null or player_a_score_140_plus >= 0),
  add column if not exists player_b_score_140_plus int check (player_b_score_140_plus is null or player_b_score_140_plus >= 0);

comment on column public.league_matches.player_a_scoring_average is '"Scoring"-gemiddelde zoals Scolia/DartCounter dat apart van het algemene gemiddelde tonen.';
comment on column public.league_matches.player_a_first9_average is 'Gemiddelde van de eerste 9 darts (3 beurten) per leg.';
comment on column public.league_matches.player_a_checkouts_hit is 'Aantal geslaagde checkouts; samen met checkout_attempts de bron voor het checkoutpercentage (niet apart opgeslagen).';
comment on column public.league_matches.player_a_score_60_plus is 'Aantal beurten met een score van 60 t/m 79 (exclusief de 80+/100+/140+/180-categorieën).';

-- report_league_match_result (sectie 7) kreeg twee extra parameters. Een
-- CREATE OR REPLACE met extra parameters (zelfs met defaults) vervangt de
-- bestaande functie niet, maar voegt een tweede, gelijknamige overload toe -
-- expliciet droppen om die dubbelzinnigheid te voorkomen.
drop function if exists public.report_league_match_result(
  uuid, uuid, int, int, numeric, numeric, int, int, int, int
);

-- p_extra_a/p_extra_b: vrije, optionele uitbreiding met de nieuwe
-- statistieken (keys: scoring_average, first9_average, checkouts_hit,
...
-- ============================================================================
-- 22. Wedstrijddeadline afgeschaft: spelers hebben geen vaste speeltermijn
--     meer per wedstrijd (was: match_deadline_days, standaard 7 dagen).
--     Alleen available_at (wanneer een wedstrijd beschikbaar komt, sectie
--     13) blijft over. generate_league_matches/on_league_activated/
--     protect_match_schedule_fields zijn hierboven al aangepast (sectie 11
--     resp. 13) om geen deadline_at meer te zetten of te bewaken;
--     extend_match_deadline()/process_match_deadlines() zijn daar al
--     verwijderd. Hier alleen nog de cron-job en de nu ongebruikte kolommen.
-- ============================================================================

select cron.unschedule(jobid) from cron.job where jobname = 'process_match_deadlines';

alter table public.league_matches
  drop column if exists deadline_at,
  drop column if exists deadline_expired_at,
...
-- ============================================================================
-- 39. Reminder for a match that's been available for a while but still
--     hasn't been played. Match deadlines were deliberately removed
--     (section 22) - players no longer have a fixed play-by date - so this
--     nudges based on available_at instead of a deadline, once per match.
--     Re-issues notif_text again with a 'match_reminder' case added.
-- ============================================================================

alter table public.league_matches
  add column if not exists stale_reminder_sent_at timestamptz;

comment on column public.league_matches.stale_reminder_sent_at is 'Gezet zodra de "nog niet gespeeld"-herinnering voor deze wedstrijd is verstuurd (voor idempotentie).';
...
begin
  ...
  insert into public.notifications (player_id, type, title, body, league_match_id)
  select due.player_a_id, 'match_reminder', nt.title, nt.body, due.id
  from due
  cross join lateral public.notif_text('match_reminder', jsonb_build_object('league_name', due.league_name)) nt
  union all
  select due.player_b_id, 'match_reminder', nt.title, nt.body, due.id
  from due
  cross join lateral public.notif_text('match_reminder', jsonb_build_object('league_name', due.league_name)) nt;
...
revoke execute on function public.remind_stale_unplayed_matches() from public, anon, authenticated;

select cron.schedule(
  'remind_stale_unplayed_matches',
  '0 8 * * *',
  $cron$select public.remind_stale_unplayed_matches();$cron$
);
